...
static  u8  s_i10msFlag  = FALSE;       //10ms标志设置为 FALSE
...
{
  static  u16 s_iCnt4   = 0;                         //定义一个静态变量s_iCnt4作为10ms计数器

  if(TIM_GetITStatus(TIM4, TIM_IT_Update) == SET)    //检查到TIM4更新中断的发生
  {
    TIM_ClearITPendingBit(TIM4, TIM_FLAG_Update);	   //清除TIM4更新中断标志
  }

  s_iCnt4++;                                         //10ms计数器的计数值加1
  
  if(s_iCnt4 >= 10)                                   //10ms计数器的计数值大于或等于10
  {
    s_iCnt4 = 0;                                     //重置10ms计数器的计数值为0
...
void InitTimer(void)
{
  ...
  ConfigTimer4(999, 71);  //72MHz /（71 + 1） = 1MHz，由0计数到999为1ms
...
{
  s_i10msFlag = FALSE;     //将2ms标志设置为FALSE 
}
