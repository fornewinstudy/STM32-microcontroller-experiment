...
*                                              内部变量
*********************************************************************************************************/
static StructCirQue s_structUARTSendCirQue;   //发送串口循环队列(注意为队列)
...
static u8 s_iUARTTxSts;                        //串口发送数据状态(为1正在发送，为0发送完成)
/*********************************************************************************************************
*                                              内部函数声明
*********************************************************************************************************/
...
static void EnableUARTTx(void);           //使能串口发送，在WriteUARTx中调用，每次发送数据之后需要调用
...
void USART1_IRQHandler(void)
{
  ...
  if(USART_GetFlagStatus(USART1, USART_IT_RXNE) != RESET)     //接收缓冲区非空中断
  {
